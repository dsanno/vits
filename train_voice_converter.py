...
def run(rank, n_gpus, hps):
  ...
  train_sampler = DistributedBucketSampler(
      ...
      rank=rank,
      shuffle=True)
  collate_fn = AudioSpeakerCollate()
  ...
  for epoch in range(epoch_str, hps.train.epochs + 1):
    if rank==0:
      train(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], scaler, train_loader, logger, writer)
    else:
      train(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], scaler, train_loader, None, None)
    scheduler_g.step()
    scheduler_d.step()
...
def train(rank, epoch, hps, nets, optims, scaler, train_loader, logger, writer):
  net_g, net_d = nets
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (spec, spec_lengths, y, y_lengths, speakers) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        if hps.model.use_flow:
          ...
          loss_kl = gaussian_kl_loss(m_q, logs_q, m_p, logs_p, z_mask) * hps.train.c_kl
