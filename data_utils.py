...
class AudioSpeakerLoader(torch.utils.data.Dataset):
    """
        ...
        3) computes spectrograms from audio files.
    """
    def __init__(self, audiopaths_sid, hparams):
        self.audiopaths_sid = load_filepaths_and_text(audiopaths_sid)
        self.max_wav_value = hparams.max_wav_value
        self.sampling_rate = hparams.sampling_rate
        self.filter_length  = hparams.filter_length
        self.hop_length     = hparams.hop_length
    ...
    def _filter(self):
        ...
        for audiopath, sid in self.audiopaths_sid:
            audiopaths_sid_new.append([audiopath, sid])
            lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length))
        self.audiopaths_sid = audiopaths_sid_new
        self.lengths = lengths
...
class AudioSpeakerCollate():
    ...
    def __call__(self, batch):
        """Collate's training batch from audio and speaker identities
        PARAMS
        ------
        ...
        if self.return_ids:
            return spec_padded, spec_lengths, wav_padded, wav_lengths, sid, ids_sorted_decreasing
        return spec_padded, spec_lengths, wav_padded, wav_lengths, sid
